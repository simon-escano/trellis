...
| --- | --- | --- | --- |
| **1. Database Schema** | Inspect tables via `psql` or DBeaver. | `documents`, `entities`, `entity_relationships` tables exist with GIN & BTree indexes. | [ ] |
| **2. Rust GraphQL API** | Query `getDocuments` in GraphiQL (`:8080/graphql`). | Returns empty array `[]` or seed records with HTTP 200. | [ ] |
| **3. UI Ingest Trigger** | Open `:4200`, click **+ Ingest Spec**, submit test payload. | Document immediately appears in sidebar with amber `QUEUED` badge. | [ ] |
| **4. Worker AI Processing** | Watch `apps/worker` terminal stream logs. | Worker picks up UUID, prompts LLM, validates Zod schema, and commits transaction. | [ ] |
| **5. DataLoader Batching** | Inspect Rust backend terminal during document load. | Single `SELECT * ... ANY($1)` query executed instead of N+1 child queries. | [ ] |
| **6. Knowledge UI Render** | Select completed document in Angular dashboard. | Summary displays, entity tags render with category colors, connection cards animate in. | [ ] |
...
* [ ] No raw API keys or secrets hardcoded in source files (verified `.env.example` in all subdirectories).
* [ ] All database queries parameterized through `sqlx` (Rust) and `pg` (TypeScript) to prevent SQL injection.
* [ ] Permissive local CORS limited strictly to `http://localhost:4200`.
* [ ] Atomic git commit log maintained across all development phases.
